...
Llega el momento decisivo: comprobar cómo responde este stack ante ráfagas concurrentes. Para ello desarrollé un script en Python que dispara ráfagas simultáneas mediante llamadas asíncronas con `asyncio` y `aiohttp` (código completo disponible en [GitHub](https://github.com/jrodriiguezg/vllm-production) y en mi servidor autoalojado en [Gitea](https://git.jrodriiguezg.link/jrodriiguezg/vllm-production)).
...
El archivo del dashboard de Grafana preconfigurado (`panel.yaml`), el orquestador Compose y el código del benchmark los tenéis disponibles en [GitHub](https://github.com/jrodriiguezg/vllm-production) y en mi servidor autoalojado en [Gitea](https://git.jrodriiguezg.link/jrodriiguezg/vllm-production).
